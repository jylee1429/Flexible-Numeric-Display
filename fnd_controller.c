...
void send(uint8_t X){ // shift register
	int i;
	for(i=0;i<8;i++){
		if (X & 0x80){ // 0b10000000
	...
	HAL_GPIO_WritePin(FND_SCLK_GPIO_Port, FND_SCLK_Pin, HIGH); // shift clock
	}
}


void send_port(uint8_t X, uint8_t port){ // compose two shift register
	send(X); // 8bit -> FND A,B,C,D,E,F,G
	send(port); // 4bit -> FND DIG 1,2,3,4
	HAL_GPIO_WritePin(FND_RCLK_GPIO_Port, FND_RCLK_Pin, HIGH);
	HAL_GPIO_WritePin(FND_RCLK_GPIO_Port, FND_RCLK_Pin, LOW); // latch clock
}
